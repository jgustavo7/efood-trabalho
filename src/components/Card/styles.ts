...
export const CardImagem = styled.div<CardProps>`
    width: 100%;
    border 1px solid transparent;
    background-color: ${(props) =>
      props.$card === 'primary' ? '' : Cores.text};
...
export const CardContent = styled.div<CardProps>`
  ...
  padding: 8px;
  background-color: ${(props) =>
    props.$card === 'primary' ? Cores.white : Cores.text};
  border-right: 1px solid;
  border-bottom: 1px solid;
  border-left: 1px solid;
  border-color: ${(props) =>
    props.$card === 'primary' ? Cores.text : 'transparent'};
`
...
export const Title = styled.h2<CardProps>`
  color: ${(props) => (props.$card === 'primary' ? Cores.text : Cores.primary)};
  font-size: 18px;
  font-weight: 700;
`
